...
class RegressionLabel(BaseKeypointCodec):
    ...
    def encode(self,
        ...
        Returns:
            ...
            - keypoint_weights (np.ndarray): The target weights in shape
                (N, K)
        """
        # print("keypoints", keypoints.shape, keypoints)
        if keypoints_visible is None:
            keypoints_visible = np.ones(keypoints.shape[:2], dtype=np.float32)
        # print("keypoints_visible", keypoints_visible.shape, keypoints_visible)
        w, h = self.input_size
        valid = ((keypoints >= 0) &
                 (keypoints <= [w - 1, h - 1])).all(axis=-1) & (
                     keypoints_visible > 0.5)
        # print("valid", valid.shape, valid)
    ...
    def decode(self, encoded: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if encoded.shape[-1] == 2:
            ...
            # print("normalized_coords",normalized_coords.shape,normalized_coords)
            output_sigma = encoded[..., 2:4].copy()
            # print("output_sigma",output_sigma.shape,output_sigma)
