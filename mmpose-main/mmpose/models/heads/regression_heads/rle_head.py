...
class RLEHead(BaseHead):
    ...
    def predict(self,
        ...
        if test_cfg.get('flip_test', False):
            ...
            # #print("网络输出",batch_coords.shape)
            batch_coords[..., 2:] = batch_coords[..., 2:].sigmoid()

        batch_coords.unsqueeze_(dim=1)  # (B, N, K, D)(batchsize,numkeypoint, , )
    ...
    def loss(self,
        ...
        #print("inputs",len(inputs),len(inputs[0]),len(inputs[0][0]))
        #print("batch_data_samples",len(batch_data_samples),batch_data_samples)

        pred_outputs = self.forward(inputs)
        #print("pred_outputs",pred_outputs.shape,pred_outputs)
        keypoint_labels = torch.cat(
            [d.gt_instance_labels.keypoint_labels for d in batch_data_samples])
        #print("keypoint_labels",keypoint_labels.shape,keypoint_labels)

        keypoint_weights = torch.cat([
            d.gt_instance_labels.keypoint_weights for d in batch_data_samples
        ])
        #print("keypoint_weights",keypoint_weights.shape,keypoint_weights)
        pred_coords = pred_outputs[:, :, :2]
        #print("pred_coords",pred_coords.shape,pred_coords)
        pred_sigma = pred_outputs[:, :, 2:4]
        #print("pred_sigma",pred_sigma.shape,pred_sigma)
        # calculate losses
        losses = dict()
