...
class CSP_CrossLayer(BaseModule):
    ...
    def forward(self, x: Tensor) -> Tensor:
        """Forward function."""
        # print("****************************")
        # print("CSP_CrossLayer入口",x.shape)
        x_13=self.conv_13(x)
        x_31=self.conv_31(x)
        min_height = min(x_13.size(2), x_31.size(2))
        min_width = min(x_13.size(3), x_31.size(3))
        # print("CSP_CrossLayer   x_13,x_31", x_13.shape, x_13.shape)
        min_height = min(x_13.size(2), x_31.size(2))
        min_width = min(x_13.size(3), x_31.size(3))
        x_13 = x_13[:, :, :min_height, :min_width]
        x_31 = x_31[:, :, :min_height, :min_width]
        # print("CSP_CrossLayer   x_13,x_31", x_13.shape, x_13.shape)
        ...
        # 进行特征图的融合，这里以按元素相加为例
        # print("CSP_CrossLayer   x_13,x_31",x_13.shape,x_13.shape)
        x_cross = torch.cat((x_13, x_31), dim=1)
        x_merge = torch.cat((x_cross, x_final), dim=1)
        # print("CSP_CrossLayer   x_merge",x_merge.shape)
        x_res=self.conv11(x_merge)
        return x_res
